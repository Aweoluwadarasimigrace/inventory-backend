...
const createPurchase = async (req, res) => {
    ...
    const {sku, productName, quantity, supplier, purchasePrice, date, received, paymentStatus, paymentMethod} = req.body;
...
const getPurchases = async (req, res) => {
    ...
    try {
        ...
        const totalPurchases = await Purchase.countDocuments({ teamAdmin: teamAdminId });
...
const {productName, supplier, received, paymentStatus, paymentMethod} = req.body;
...
if (req.user.role === "admin") {
  ...
  try {
    const deletePurchase = await Purchase.findOneAndDelete({ _id: id, teamAdmin: teamAdminId });
    if (!deletePurchase) {
      return res.status(404).json({ message: "Purchase not found" });
    }
